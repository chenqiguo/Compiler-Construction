...
namespace {
  struct CAT : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
      ...
      for (auto i : LI){
        auto loop = &*i;
        analyzeLoop(loop, DT, "#", false, loopID);
      }
      return false;
    }
    ...
    private: 
    void analyzeLoop (Loop *loop, DominatorTree &DT, string prefix, bool printBasicBlocks, int &loopID){
      ...
      for (auto j : subLoops){
        ...
        analyzeLoop(subloop, DT, prefix + '#', printBasicBlocks, loopID);
      }
