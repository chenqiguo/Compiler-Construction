...
namespace {
  struct CAT : public FunctionPass {
    ...
    ConstantInt * constantFolding (Instruction *i, vector<Instruction *> &insts, SparseBitVector<> &ins) {
      CallInst *callInst = cast<CallInst>(i);
      Function *callee = callInst->getCalledFunction();
      Instruction *reachDef1 = NULL; // for constant folding
      Instruction *reachDef2 = NULL; // for constant folding
      bool isSet1 = 0; // for constant folding
      bool isSet2 = 0; // for constant folding
      ConstantInt *constant = NULL; // return val
      errs() << "HERE1!!!\n";
      ...
      if ((callee->getName() == "CAT_add") || (callee->getName() == "CAT_sub")) { // constant folding for CAT_add and CAT_sub
        ...
        for (auto &&in : ins) { // note that the IN set is in order of the original instruction!
          ...
          if (CallInst *in_callInst = dyn_cast<CallInst>(inInst)) {
            ...
            if (in_callee->getName() == "CAT_new") {
              if (inInst == inst_arg1) reachDef1 = inInst; // may replace v1 with constant
              else if (inInst == inst_arg2) reachDef2 = inInst; // may replace v2 with constant
              errs() << "HERE5-1!!!\n";
            }
        ...
        if (reachDef1 && reachDef2) { // only can fold when both v1 and v2 are constants!
          Value *val1; //v1
          Value *val2; //v2
          errs() << "HERE9!!!\n";
          if (CallInst *reachDef1_callInst = dyn_cast<CallInst>(reachDef1)) {
            if (CallInst *reachDef2_callInst = dyn_cast<CallInst>(reachDef2)) {
              errs() << "HERE10!!!\n";
              if (!isSet1) {
                val1 = reachDef1_callInst->getArgOperand(0);
              }
              if (!isSet2) {
                val2 = reachDef2_callInst->getArgOperand(0);
              }
              if (isSet1) {
                val1 = reachDef1_callInst->getArgOperand(1);
              }
              if (isSet2) {
                val2 = reachDef2_callInst->getArgOperand(1);
              }
              if (ConstantInt *constant1 = dyn_cast<ConstantInt>(val1)) {
                if (ConstantInt *constant2 = dyn_cast<ConstantInt>(val2)) {
                  ...
                  errs() << "HERE11!!!\n";
                  int64_t result = constant1->getSExtValue() + constant2->getSExtValue();
                  constant = ConstantInt::get(constant1->getType(), result); //???
                  errs() << "inst is: " << *i << "\n",
                  errs() << "constant is: " << *constant << "\n";
                  errs() << "constant1 is: " << *constant1 << "\n";
                  errs() << "constant2 is: " << *constant2 << "\n";
                  //BasicBlock::iterator ii(i); //???bug???*i
                  //BasicBlock *bb = i->getParent();
                  //ReplaceInstWithValue(bb->getInstList(), ii, constant); //???
                  //ReplaceInstWithValue(bb->getInstList(), ii, constant1); //???bug?constVal?
                  //ReplaceInstWithValue(bb->getInstList(), ii, constant2); //???bug?constVal?

                  errs() << "inst after constant folding: " << *i << "\n";
                  errs() << "result of constant folding: " << to_string(result) << "\n\n";
                }
              }
            }
          }
    ...
    bool runOnFunction (Function &F) override {
      ...
      // constant folding for CAT_add and CAT_sub
      for (int idx = 0; idx < insts.size(); idx++) {
        ...
        bool needFold = false; // flag for need of constant folding
        bool flag_debug = 0; // for debug
        bool isSet = 0; // for constant propagation
        Instruction *reachDef = NULL; // for constant propagation
        ConstantInt *foldConst = NULL;
        ConstantInt *resultConstant;
        int64_t foldResult;
        int64_t reachResult;
        ...
        if (callee->getName() == "CAT_get") { // constant propagation for CAT_new and CAT_set
            ...
            if (reachDef) {
              ...
              if (CallInst *reachDef_callInst = dyn_cast<CallInst>(reachDef)) {
                ...
                if (ConstantInt *constant = dyn_cast<ConstantInt>(val)) {
                  errs() << "inst before reach def is: " << *i << "\n";
                  if (flag_debug) {
                    errs() << "WTF???\n";
                    errs() << "constant is: " << *constant << "\n";
                  }
                  ...
                  //BasicBlock *bb = i->getParent();
                  //ReplaceInstWithValue(bb->getInstList(), ii, constant);
                  //errs() << "inst after reach def is: " << *i << "\n\n";
                }
                if (flag_debug) {
                  errs() << "HERE3!!!\n";
                  errs() << "reachDef is: " << *reachDef << "\n";
              ...
              //BasicBlock::iterator ii(i); //???bug???*i
              //BasicBlock *bb = i->getParent();
              //ReplaceInstWithValue(bb->getInstList(), ii, foldConst);
              //errs() << "inst after reach def is: " << *i << "\n\n";
            }
          }
          ...
          if (consts.size() == 1) {
            ...
            errs() << "Can replace the CAT var with constant: " << *resultConstant << "\n";
            BasicBlock::iterator ii(i); //???bug???*i
            BasicBlock *bb = i->getParent();
