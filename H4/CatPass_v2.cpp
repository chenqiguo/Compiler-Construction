  ...
  struct CAT : public FunctionPass {
    ...
    bool runOnFunction (Function &F) override {
      ...
      for (int idx = 0; idx < insts.size(); idx++) {
        ...
        else if ((callee->getName() == "CAT_add") || (callee->getName() == "CAT_sub")) { // constant folding for CAT_add and CAT_sub
              ...
              if (CallInst *reachDef2_callInst = dyn_cast<CallInst>(reachDef2)) {
                ...
                if (ConstantInt *constant1 = dyn_cast<ConstantInt>(val1)) {
                  if (ConstantInt *constant2 = dyn_cast<ConstantInt>(val2)) {
                    ...
                    ConstantInt *constant = ConstantInt::get(constant1->getType(), result); //???
                    errs() << "inst is: " << *i << "\n",
                    errs() << "constant is: " << *constant << "\n";
