  ...
  struct CAT : public FunctionPass {
    ...
    bool analyzeLoop(LoopInfo &LI, Loop *loop, DominatorTree &DT, ScalarEvolution &SE, AssumptionCache &AC, OptimizationRemarkEmitter &ORE /*, int loopSize*/) {
      ...
      if (loop->isLoopSimplifyForm() && loop->isLCSSAForm(DT)) {
        ...
        // // for test55
        // // before unroll the loop, loop through the instructions within this loop;
        // // if there is only one CAT var and the belowed CAT_get to get it, replace the CAT var with const
        ...
        // // reference from Piazza "One way of knowing when to stop unrolling"
        // // set metadata to imply "already unrolled"
        // loop->setLoopAlreadyUnrolled();
        // // check metadata to see if "already unrolled"
        // MDNode* loopId = loop->getLoopID(); // get metadata node of loop
        // MDNode* DisableUnrollMD = GetUnrollMetadata(loopId, "llvm.loop.unroll.disable"); // get metadata
        // // if either of above are nullptr, then it means your loop hasn't been unrolled. 
        // // else it has been unrolled
        // if ((loopId != nullptr) && (DisableUnrollMD != nullptr)) return false;
