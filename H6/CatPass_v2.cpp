  ...
  struct CAT : public FunctionPass {
    ...
    bool constProp(vector<Instruction *> &insts, Instruction *inst_arg0, vector<SparseBitVector<>> &INs, set<Instruction*> &escapeSet, int idx) {
      ...
      if (auto arg0_callInst = dyn_cast<CallInst>(inst_arg0)) {
        errs() << "HERE2\n";
        errs() << "arg0_callInst: " << *arg0_callInst << "\n";
    ...
    bool constantPropagation(vector<Instruction *> &insts, vector<SparseBitVector<>> &INs, set<Instruction*> &escapeSet, int idx, Value *arg0Value, AliasAnalysis &aliasAnalysis) {//auto???
    ...
    bool aliasDepAnalysis(vector<Instruction *> &insts, vector<SparseBitVector<>> &INs, AliasAnalysis &aliasAnalysis, MemoryDependenceResults &MD) {
      ...
      // and just give up on optimizing the CAT vars that is arg or in memSet (here escapeSet) that are aliased or depends on other CAT vars
