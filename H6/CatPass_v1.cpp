...
namespace {
  struct CAT : public FunctionPass {
    ...
    bool dealEscape(vector<Instruction *> &insts, vector<SparseBitVector<>> &INs) {
      ...
      // loop through each instruction to fill in the escapeSet
    ...
    void computeMemArg(vector<Instruction *> &insts, set<Instruction*> &memSet, set<Instruction*> &argSet) {
      /*HW6*/
      // compute the escapeSet (memSet and argSet), used in aliasDepAnalysis()
    ...
    void computeEscape(vector<Instruction *> &insts, set<Instruction*> &argSet, set<Instruction*> &memSet, set<Instruction*> &escapeSet, AliasAnalysis &aliasAnalysis, MemoryDependenceResults &MD) {
      /*HW6*/
      // compute CAT vars that are on memory and definitely alias or depend on others (intra!) <- called escapeSet
      // ???memSet???

      // reference: LLVM_dependences.tar.bz2, llvm0: intra, alias analysis
      ...
      // CANNOT USE THIS!!!!!
      // find alias insts: conservative, contains MayAlias, MustAlias, PartialAlias
      // (i.e., only exclude NoAlias)
      // for (auto &pointer : pointers) {
      ...
      //         escapeSet.insert(pointer);
      //         escapeSet.insert(pointer2);
      //         break;
      //       default:
      //         abort();
      //     }
        ...
        for (auto &memInst2 : memInsts) {
          ...
          switch (aliasAnalysis.alias(MemoryLocation::get(memInst), MemoryLocation::get(memInst2))) {
            case NoAlias:
              break;
            case MayAlias: case PartialAlias: case MustAlias:
              escapeSet.insert(memInst);
              escapeSet.insert(memInst2);
              break;
            default:
              abort();
          }
      ...
      // CANNOT USE THIS!!!!!
      // find instructions that can modify (mod) or read (ref) a memory location:
      // conservative, contains MRI_Mod, MRI_Ref, MRI_ModRef (i.e., only exclude MRI_NoModRef)
      // for (auto &memInst : memInsts) {
      //   for (auto &pointer : pointers) {
      //     auto sizePointer = getPointedElementTypeSize(pointer);
      ...
      // reference: LLVM_dependences.tar.bz2, llvm2: intra, dependence analysis
      vector< Instruction* > memInsts_dep;
      for (auto i : insts) {
        if (isa<LoadInst>(i) || isa<StoreInst>(i)) {
          memInsts_dep.push_back(i);
          continue ;
        }
      }
      for (auto &memInst : memInsts_dep) {
      ...
      for (auto arg : argSet) {
        escapeSet.insert(arg); // set of CAT vars that cannot be optimized
      }

      for (auto mem : memSet) {
        escapeSet.insert(mem); // set of CAT vars that cannot be optimized
      }
    ...
    bool aliasDepAnalysis(vector<Instruction *> &insts, vector<SparseBitVector<>> &INs, AliasAnalysis &aliasAnalysis, MemoryDependenceResults &MD) {
      ...
      bool modified = false; // the return value
      errs() << "******************aliasDepAnalysis******************\n";
      // escape: 1) CAT vars as parameters of a function; 2) CAT vars stored in memory
      // we only optimize CAT vars in memSet that are not aliased or depends on other CAT vars,
      // and just give up on optimizing the CAT vars in argSet or in memSet that are aliased or depends on other CAT vars
      set<Instruction*> memSet; // set of escaped CAT vars that are on memory
      set<Instruction*> argSet; // set of escaped CAT vars that are from func arguments
      computeMemArg(insts, memSet, argSet);
      if (debug) {
        errs() << "argSet:\n";
        for (auto arg: argSet) {
          errs() << *arg << "\n";
      ...
      computeEscape(insts, argSet, memSet, escapeSet, aliasAnalysis, MD); // escapeSet cannot be optimized!!!
      // for (auto arg : argSet) {
      //   escapeSet.insert(arg); // set of CAT vars that cannot be optimized
      // }
      //???
    ...
    // bool depAnalysis(vector<Instruction *> &insts, vector<SparseBitVector<>> &INs, MemoryDependenceResults &MD, bool modified) { //MD???
    //   /*HW6*/
    //   // reference: Dependences slides
    //   // modified from dealEscape() in HW5
